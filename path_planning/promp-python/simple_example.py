...
# # with open('promp-python/examples/strike_mov.npz','r') as f:
# data = np.load('promp-python/examples/strike_mov.npz', allow_pickle=True, encoding='bytes')  
# time = data['time'][0:10] 
# print("Q shape :", np.array(data['Q'][0]).shape)  
# Q = data['Q'][0:10] 
# print("Q :", len(Q))  
# # print("time :", time) 
...
# print("Q :", Q[0])      
print("demos shape :", demos[0].shape, len(Q))    
...
time = np.squeeze(demos_t)   
print("time :", len(time)) 

#2) Create a ProMP with basis functions: 3 RBFs with scale 0.25 and 
#   centers 0.25, 0.5 and 0.75. 
#   Use also polynomial basis functions of degree one (constant and linear term)  
full_basis = {
        'conf': [
...
print("prior_sigma_w :", prior_Sigma_w)  
train_summary = robot_promp.train(time, q=Q, max_iter=10, prior_Sigma_w=prior_Sigma_w, print_inner_lb=True)  

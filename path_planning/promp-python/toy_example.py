...
def create_toy_data(n=100, T=30, missing_obs=[40,40]):
    ...
    if missing_obs is not None:
        for i in range(missing_obs[0]): 
            times[i] = np.delete(times[i], range(1,T//2))
        for i in range(missing_obs[1]): 
            times[i+missing_obs[0]] = np.delete(times[i+missing_obs[0]], range(T//2,T-1))

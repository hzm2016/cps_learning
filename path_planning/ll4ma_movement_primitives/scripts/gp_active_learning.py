...
class GPActiveLearning:

    def __init__(self, foldername, num_frames=5, grid_dim=20, neg_starts=[], neg_extents=[],
                 random_sample=False, neg_border_width=0):
        self.folder_path = os.path.expanduser("~/Videos/gp_active_learning/%s" % foldername)
        try:
        ...
        self.gp = GaussianProcessClassifier(kernel=RBF(length_scale=2.0), optimizer=None)
        self.num_samples = 0
        self.num_neg = 0
        self.num_pos = 0
        self.tot_num_neg = 0
    ...
    def run(self):
        print ("\n\n===========================================================================\n"
               "  Running Gaussian Process active learning:\n"
               "    Folder: %s\n"
               "    Num Iterations: %d\n"
               "    Random Sample: %r\n"
               "===========================================================================\n\n"
        ...
        pos_data = self.candidates[np.where(self.candidates[:,0] == 1)]
        neg_data = self.candidates[np.where(self.candidates[:,0] == 0)]
        pos_samples = pos_data[np.random.randint(pos_data.shape[0], size=1),:]
        neg_samples = neg_data[np.random.randint(neg_data.shape[0], size=1),:]
        ...
        self.gp.fit(self.samples[:,1:], self.labels)
    ...
    def animate(self, itr_num):
        ...
        full_probs = self.gp.predict_proba(self.full_data[:,1:])
        full_entropies = self.get_entropies(full_probs, self.full_data)

        # Get the next sample
        probs = self.gp.predict_proba(self.candidates[:,1:])

        entropies = self.get_entropies(probs, self.candidates)
        ...
        self.samples = np.vstack((self.samples, next_sample))
        self.labels.append(next_sample[0])
        if next_sample[0] == 1:
            self.num_pos += 1
        else:
            self.num_neg += 1
    ...
    def plot(self, next_sample, entropies):
        ...
        pos_samples = self.samples[np.where(self.samples[:,0] == 1)]
        neg_samples = self.samples[np.where(self.samples[:,0] == 0)]
        
        self.plot_scatter_data(pos_samples[:,1:], axes[0], "cornflowerblue")
        self.plot_scatter_data(neg_samples[:,1:], axes[0], "firebrick")
    ...
    def get_entropies(self, probs, data):
        entropies = []
        for i in range(probs.shape[0]):
            e = self.entropy(probs[i][0], probs[i][1])
            entropies.append((data[i,:], e))
        return entropies
...
if __name__ == '__main__':
    ...
    neg_extents = [5, 5]
    learner = GPActiveLearning(args.foldername, args.num_frames, args.grid_dim, neg_starts,
                               neg_extents, args.random_sample, args.neg_border_width)
    learner.run()
